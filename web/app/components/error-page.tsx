...
export function ErrorPage({
    ...
    return (
        <div className="flex items-center justify-center h-full px-4 py-12">
            <Empty className="border-none gap-8">
                <EmptyMedia>
                    <span
                        className={cn(
                            "font-black tracking-tighter select-none tabular-nums leading-none",
                            is404 ? "text-9xl text-foreground/10" : "text-8xl text-destructive/15",
                        )}
                    >
                        {code}
                    </span>
                ...
                <EmptyHeader>
                    {!is404 && <TriangleAlertIcon className="size-8 text-destructive/60 mb-1" />}
                ...
                {stack && (
                    <div className="w-full mt-2">
                        <div className="rounded-lg border bg-muted/50 overflow-hidden">
                            ...
                            <pre className="p-4 text-xs font-mono text-muted-foreground overflow-x-auto leading-relaxed whitespace-pre-wrap break-all text-start">
                                {stack}
                            </pre>
                        </div>
                    </div>
